...
class ModDownloader:
    ...
    def __init__(
        self,
        priority: DownloadPriority = DownloadPriority.EQUAL,
    ...
    def search_mod(
        ...
        # 从CurseForge搜索
        curseforge_results = self._search_curseforge(
            query, game_version, mod_loader, limit
        )
    ...
    def _search_modrinth(
        ...
        try:
            ...
            if self.use_mirror and self.list_priority == DownloadPriority.MIRROR_FIRST:
                ...
                self.use_mirror
                and self.list_priority == DownloadPriority.OFFICIAL_FIRST
            ):
                    ...
                    if latest_version_id:
                        ...
                        if latest_version:
                            ...
                            if (
                                ...
                                and len(latest_version["files"]) > 0
                            ):
                            ...
                            # 获取支持的游戏版本和加载器
                            mod.game_versions = latest_version.get(
    ...
    def _search_curseforge(
        ...
        has_mirror_api = self.use_mirror and bool(
            self.mirror_endpoints.get("curseforge_api")
        )
        ...
        try:
                ...
                try:
                    ...
                    if u.startswith("https://api.curseforge.com"):
                        if not self.curseforge_api_key:
                            # 无法请求官方 API，跳过
                            print(
                                "警告: 未设置CurseForge API密钥，跳过官方 CurseForge 请求"
                            )
                            continue
                        headers = {"x-api-key": self.curseforge_api_key}

                    response = self._request_with_retry(
                        u, headers=headers, params=params
                    )
            ...
            if response.status_code == 200:
                ...
                for item in data.get("data", []):
                    ...
                    mod.followers = item.get(
                        "totalDownloads", 0
                    )  # CurseForge没有直接的follower计数
                    mod.icon_url = item.get("logo", {}).get("url", "")
                    mod.official_url = (
                        f"https://www.curseforge.com/minecraft/mc-mods/{item['slug']}"
                    )
                    ...
                    if latest_files and len(latest_files) > 0:
                        ...
                        if mod_loader:
                            if any(
                                mod_loader.lower() in version.lower()
    ...
    def _get_mirror_url(self, original_url: str) -> str:
        ...
        try:
            modrinth_api = self.mirror_endpoints.get("modrinth_api")
            modrinth_cdn = self.mirror_endpoints.get("modrinth_cdn")
            curseforge_cdn = self.mirror_endpoints.get("curseforge_cdn")

            if (
                "api.modrinth.com" in original_url
                or "staging-api.modrinth.com" in original_url
            ):
                if modrinth_api:
                    return original_url.replace(
                        "https://api.modrinth.com", modrinth_api
                    ).replace("https://staging-api.modrinth.com", modrinth_api)
            elif (
                "cdn.modrinth.com" in original_url
                or "data.modrinth.com" in original_url
            ):
                if modrinth_cdn:
                    return original_url.replace(
                        "https://cdn.modrinth.com", modrinth_cdn
                    ).replace("https://data.modrinth.com", modrinth_cdn)
            elif (
                "edge.forgecdn.net" in original_url
                or "media.forgecdn.net" in original_url
            ):
                if curseforge_cdn:
                    return original_url.replace(
                        "https://edge.forgecdn.net", curseforge_cdn
                    ).replace("https://media.forgecdn.net", curseforge_cdn)
            elif "api.curseforge.com" in original_url:
                curseforge_api = self.mirror_endpoints.get("curseforge_api")
                if curseforge_api:
                    return original_url.replace(
                        "https://api.curseforge.com", curseforge_api
                    )
        except Exception:
            pass
        return original_url
    ...
    def get_download_urls(self, mod: ModInfo) -> List[str]:
        ...
        if self.download_priority == DownloadPriority.MIRROR_FIRST:
            # 镜像优先（去重且保持顺序）
            ordered = [u for u in mirror_urls if u] + [
                u for u in official_urls if u not in mirror_urls
            ]
        elif self.download_priority == DownloadPriority.OFFICIAL_FIRST:
            ordered = [u for u in official_urls if u] + [
                u for u in mirror_urls if u not in official_urls
            ]
        else:
