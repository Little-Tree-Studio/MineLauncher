...
def shader_download_page(page: ft.Page):
    ...
    search_field = ft.TextField(
        label="搜索光影包", width=600, border=ft.InputBorder.UNDERLINE
    )
    ...
    shader_list = ft.ListView(expand=True, spacing=10)
    ...
    def show_shader_intro(shader_name, shader_desc):
        dlg = ft.AlertDialog(
            title=ft.Text(f"{shader_name} 简介"),
            content=ft.Text(shader_desc or "暂无简介"),
    ...
    def search_shaders(e=None):
        ...
        try:
            ...
            shader_list.controls.clear()
            shader_list.controls.append(ft.Text(f"网络连接失败: {ex}"))
            page.update()
            return
        shader_list.controls.clear()
        if resp.status_code == 200:
            for shader in resp.json()["hits"]:
                ...
                try:
                    ...
                    shader_list.controls.append(ft.Text(f"获取版本信息失败: {ex}"))
                    continue
                version_options = []
                version_files = []
                ...
                async def on_download_click(
                    ...
                    if folder_path:
                        download_shader(file_url, file_name, page, pb, folder_path)

                shader_item = ft.Container(
                    content=ft.Row(
                        [
                                    ...
                                    ft.Row(
                                        [
                                            ...
                                            ft.IconButton(
                                                ...
                                                on_click=(
                                                    lambda n=name,
                                                    d=desc: lambda e: show_shader_intro(
                                                        n, d
                                                    )
                                                )(),
                                            ),
            ...
            shader_list.controls.append(ft.Text("搜索失败，请重试。"))
            page.update()
